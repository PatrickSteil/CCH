...
std::vector<T> load_vector(const std::string& file_name)
{
    ...
    if (file_size % sizeof(T) != 0)
        throw std::runtime_error(
            "File \"" + file_name + "\" can not be a vector of the requested type because it's size is no "
                                    "multiple of the element type's size.");
...
template <>
void save_vector<std::string>(const std::string& file_name,
...
template <>
std::vector<std::string> load_vector<std::string>(
    const std::string& file_name)
{
    std::vector<char> data = load_vector<char>(file_name);
    std::vector<std::string> ret;
    std::vector<char>::const_iterator str_begin = data.begin(),
                                      str_end = data.begin(),
                                      data_end = data.end();
